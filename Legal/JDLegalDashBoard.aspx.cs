...
public partial class Legal_JDLegalDashBoard : System.Web.UI.Page
{
    DataSet ds;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (Session["Emp_ID"] != null)
            {
                if (!IsPostBack)
                {
                    int OicId = Convert.ToInt16(Session["District_ID"].ToString());
    ...
    //    dsCasecount = objdb.ByProcedure("Sp_LegalDivisionWiseCasesDashboard", new string[] { "OicId" }, new string[] { Convert.ToString(OicId) }, "dataset");
    //    string str = "";
    ...
    protected void CourtTypeCase(int OicId)
    {
        try
        {
            ...
            dsCase = objdb.ByProcedure("Sp_LegalDivisionWiseCasesDashboard", new string[] { "OicId" }, new string[] { Convert.ToString(OicId) }, "dataset");
    ...
    protected void CourtTypeCase1(int OicId)
    {
        try
        {
            int CaseCount1 = 0;
            DataSet dsCase = new DataSet();
            dsCase = objdb.ByDataSet("Select CourtName as Court,COUNT(distinct UniqueNo) OrderBydirectCourtWiseCount from tblLegalCaseRegistration where CaseDisposalType_Id='2' and CourtLocation_Id=" + OicId + " and Isactive=1  group by CourtName");
...
"COUNT(distinct UniqueNo) CaseSubjectWiseCount from tblLegalCaseRegistration a where a.CourtLocation_Id=" + OicId + "" +
"group by a.CaseSubject_Id");
    ...
    protected void CourtTypeCase3(int OicId)
    {
        try
        {
            int CaseCount1 = 0;
            DataSet dsCase = new DataSet();
            dsCase = objdb.ByDataSet("Select  case when Compliance_Status = 1 then 'Yes' else 'No' end as IsComplaince,COUNT(distinct UniqueNo ) OrderBydirectComplaincesStatus from tblLegalCaseRegistration where CaseDisposalType_Id='2'  and District_ID=" + OicId + " group by Compliance_Status");
            StringBuilder Sb1 = new StringBuilder();
            Sb1.Append("<script type='text/javascript' src='https://www.gstatic.com/charts/loader.js'></script>");
            Sb1.Append("<script type='text/javascript'>");
            Sb1.Append(" google.charts.load(");
    ...
    protected void BIndWACaseCount(int OicId)
    {
        try
        {
            ds = objdb.ByProcedure("Sp_LegalDivisionWiseCasesDashboard", new string[] { "OicId" }, new string[] { Convert.ToString(OicId) }, "dataset");
