            ...
            if (Page.IsValid)
            {
                ...
                if (Session["Role_ID"].ToString() == "4")//District Login
                {
                    ...
                    ds = obj.ByProcedure("USP_GetWPPendingRpt", new string[] { "Casetype_ID", "CourtLocation_Id", "flag", "Court_Id", "CaseNo", "Fromdate", "Todate"}
                        , new string[] { ddlCasetype.SelectedValue, hdnDistrict_Id.Value, "2", ddlCourtName.SelectedValue, ddlCaseNo.SelectedItem.Text, FromDate, Todate }, "dataset");
                }
                else if (Session["Role_ID"].ToString() == "2")
                {
                    //string Division_Id = Session["Division_Id"].ToString();
                    ds = obj.ByProcedure("USP_GetWPPendingRpt", new string[] { "Casetype_ID", "CourtLocation_Id", "flag", "Court_Id", "CaseNo", "Fromdate", "Todate" }
                       , new string[] { ddlCasetype.SelectedValue, District_Id, "3", ddlCourtName.SelectedValue, ddlCaseNo.SelectedItem.Text, FromDate, Todate }, "dataset");
                }
                else if (Session["Role_ID"].ToString() == "5")
                {
                    //string District_Id = Session["District_Id"].ToString();
                ...
                else
                {
                    ...
                    ds = obj.ByProcedure("USP_GetWPPendingRpt", new string[] { "Casetype_ID", "OICMaster_Id", "flag", "Court_Id", "CaseNo", "Fromdate", "Todate", "CourtLocation_Id" }
                        , new string[] { ddlCasetype.SelectedValue, OIC, "1", ddlCourtName.SelectedValue, ddlCaseNo.SelectedItem.Text, FromDate, Todate,District_Id }, "dataset");
                }
                if (ds != null)
                {
    ...
    protected void ddlCourtName_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ds1 != null && ds1.Tables[1].Rows.Count > 0)
            {
                ...
                {
                    //ddlDistrict.Attributes["class"] = "ddldisable";
